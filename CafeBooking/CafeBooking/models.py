...
class User(models.Model):
    role = models.ForeignKey(Role, on_delete=models.CASCADE, null=False, verbose_name="Роль")
...
class Place(models.Model):
    short_name = models.CharField(max_length=5, null=False, verbose_name="Короткое название")
    office = models.ForeignKey(Office, on_delete=models.CASCADE, null=False, verbose_name="Офис")
    
    
class Type(models.Model):
    name = models.CharField(max_length=20, verbose_name="Тип")
    place = models.ForeignKey(Place, on_delete=models.CASCADE, null=False, verbose_name="Место")
...
class Reservation(models.Model):
    ...
    time = models.ForeignKey(Time, on_delete=models.CASCADE, null=False, verbose_name="Время")
    status = models.ForeignKey(Status, on_delete=models.CASCADE, null=False, verbose_name="Статус")
    type = models.ForeignKey(Type, on_delete=models.CASCADE, null=False, verbose_name="Место")
    date = models.DateField(null=False, verbose_name="Дата")
    cancel_reason = models.CharField(max_length=100, null=True, verbose_name="Причина отмены")
    
    is_archivated = models.BooleanField(verbose_name="Архивированная запись", default=False, blank=False)
    ...
    def check_continue(self):
        ...
        if self.time.time == "Утреннее":
            ...
            if Reservation.objects.filter(time__time="Вечернее", date=self.date, status=status, type=self.type).exists():
                return False
            return True
        else:
            return False
...
class Setting(models.Model):
    months_to_archive = models.IntegerField(verbose_name="Срок хранения данных", default=2, validators=[MinValueValidator(1, "Нельзя поставить значение меньше 1")])