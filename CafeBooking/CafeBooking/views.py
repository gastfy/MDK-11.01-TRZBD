...
def create_reservation(
    user: User, time: Time, status: Status, place: Type, date: datetime.date
):
...
def book_qr_code(request, id):
    ...
    is_booked = (
        Reservation.objects.exclude(is_archivated=True)
        .filter(time__time=time, date=today, type=place, status__status="Забронировано")
        .exists()
    )
    ...
    create_reservation(
        user,
        Time.objects.filter(time=time).first(),
        Status.objects.filter(status="Забронировано").first(),
        place,
        today,
    )
...
def __download_qrcodes(request):
    ...
    for office in offices:
        ...
        for place in places:
            ...
            draw.text(
                text_position,
                f"{"Стол" if place.name == "Стол" else "Комната"}: "
            ...
            try:
                img.save(
                    f"{cwd}\\qr-codes\\office {office.pk}\\{place.place.short_name}.png"
                )
                data.append(
                    {
                        "image": f"{cwd}\\qr-codes\\office {office.pk}\\{place.place.short_name}.png",
                        "office": office.address,
                        "place": place.place.short_name,
                    }
                )
...
def user_reservations(request):
    ...
    history = list(
        Reservation.objects.exclude(is_archivated=True)
        .filter(user=user, status__in=[closed, canceled])
        .all()
    )
    active = []
...
def reservations(request):
    ...
    if request.method == "POST":
        ...
        form = ReservationFilterForm(
            initial={"office": office, "date": date, "time": time}
        )
    ...
    reservations = (
        Reservation.objects.exclude(is_archivated=True)
        .filter(date=date, time__time=time)
    ...
    args = {
        ...
        "table_available": tables_available,
        "rooms_unavailable": rooms_unavailable,
        "table_unavailable": tables_unavailable,
        "office": office.pk,
        "date": date.strftime("%d.%m.%Y"),
        "time": time,
    }
...
def add_entity(request):
    ...
    if entity == "users":
        ...
        form = (
            ReservationForm()
            if request.method != "POST"
            else ReservationForm(request.POST)
        )
...
def create_dump(request):
    ...
    setting = Setting.objects.first()
    two_months_ago = datetime.datetime.now() - relativedelta(
        months=setting.months_to_archive
    )

    reservations = (
        Reservation.objects.filter(creation_time__lt=two_months_ago)
        .exclude(is_archivated=True)
        .all()
    )
...
def edit_entity(request):
    ...
    if entity == "users":
        ...
        form = (
            OfficeForm(instance=entity_inst)
            if request.method != "POST"
            else OfficeForm(request.POST)
        )
    elif entity == "places":
        ...
        form = (
            ReservationForm(instance=entity_inst)
            if request.method != "POST"
            else ReservationForm(request.POST)
        )
...
def admin_page(request):
    ...
    if entity == "users":
        ...
        data = list(
            Reservation.objects.exclude(is_archivated=True).all().order_by("pk")
        )
    elif entity == "logs":
        ...
        if graphic == "days":
            ...
            exclude_status = Status.objects.filter(status="Отменено").first()
            reservations = (
                Reservation.objects.exclude(status=exclude_status)
                .exclude(is_archivated=True)
                .all()
            )
            ...
            cnt = OrderedDict(
                sorted(cnt.items(), key=lambda kv: kv[1], reverse=True)[:10]
            )
    ...
    setting = Setting.objects.first()
    initial_dict = {"months_to_archive": setting.months_to_archive}
...
def login_page(request):
    ...
    if User.objects.filter(role=role).exists() is False:
        ...
        admin.save()
        log(
            f"Пользователь с id: '{admin.pk}' и ролью: '{admin.role.decrypt()}' создан!"
        )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            h_password = hasher.hexdigest()
            if (
                User.objects.filter(login=data["login"])
                ...
                if user.role.name == "Admin":
                    log(
                        f"Пользователь с id: '{user.pk}' с ролью: '{user.role.decrypt()}' вошел в систему!"
                    )
                    return redirect("/admin/")
                else:
                    log(
