...
local_time_morning = timezone.datetime(
...
local_time_evening = timezone.datetime(
    year=timezone.now().year,
    month=timezone.now().month,
    day=timezone.now().day,
    hour=19,
...
local_time_end = timezone.datetime(
    year=timezone.now().year,
    month=timezone.now().month,
    day=timezone.now().day,
    hour=23,
...
@db_periodic_task(
    crontab(hour=f"{utc_time_morning.hour}", minute=f"{utc_time_morning.minute}"),
    retries=2,
    retry_delay=240,
)
def morning_check():
    ...
    reservations = Reservation.objects.filter(
        status=reserved_status, date=today, time=time
    )
...
@db_periodic_task(
    crontab(hour=f"{utc_time_evening.hour}", minute=f"{utc_time_evening.minute}"),
    retries=2,
    retry_delay=240,
)
def evening_check():
...
@db_periodic_task(
    crontab(hour=f"{utc_time_end.hour}", minute=f"{utc_time_end.minute}"),
    retries=2,
    retry_delay=240,
)
