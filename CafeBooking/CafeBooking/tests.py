...
class TestCases(TestCase):
    ...
    def login(self, login, password):
        ...
        if User.objects.filter(login = login).filter(password = h_password).filter(is_blocked = False).exists():
            return User.objects.filter(login = login).filter(password = h_password).filter(is_blocked = False).get()
        else:
            return None
